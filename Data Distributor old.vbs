...
Do Until objExcel.Cells(rows,1).Value =  ""
 ...
 ElseIf(objExcel.Cells(rows,1).Value =  "HOME DIRECTORY - CREATION [Add]") Then
  ...
  Do Until words > 150 OR IE.document.getElementsByTagName("font").Item(words).InnerText =  "-"
   ...
   Else
    If(words < 4 OR words > 10) Then
     objSheet2.Cells(row, col).Value = Trim(Replace(Replace(Replace(Replace(Replace(Replace(Replace(Replace(IE.document.getElementsByTagName("font").Item(words).InnerText,"<"," "),">"," "),"(UID="," "),")", " "),"option:"," "),"Action:"," "),"Select application:"," "),"Select the appropriate role:"," "))
     objHome.ActiveWorkbook.Save
     col = col+1
    End If
    words= words +1
 ...
 ElseIf(Left(objExcel.Cells(rows,1).Value, 17) =  "DOCIT - QSD [Add]") Then
 ...
 ElseIf(objExcel.Cells(rows,1).Value =  "DEVLIMS [Add]") Then
  objSheete.Cells(rows, 1).Value = objExcel.Cells(rows,1).Value
  objSheete.Cells(rows, 2).Value = objSheet.Cells(rows, 2).Value
  objesetup.ActiveWorkbook.Save
  IE.Visible = 1
 ...
 ElseIf(Left(objExcel.Cells(rows,1).Value, 25) =  "DIGITAL CERTIFICATE [Add]") Then
 ...
 ElseIf(objExcel.Cells(rows,1).Value =  "AUTO DEPARTURES [Delete]" OR objExcel.Cells(rows,1).Value =  "DEPARTURES [Delete]" OR objExcel.Cells(rows,1).Value =  "DEPARTURES - AUTO [Delete]" OR objExcel.Cells(rows,1).Value =  "DEPARTURE - BP [Delete]") Then
  objSheete.Cells(rows, 1).Value = objExcel.Cells(rows,1).Value
  objSheete.Cells(rows, 2).Value = objSheet.Cells(rows, 2).Value
  objesetup.ActiveWorkbook.Save
  IE.Visible = 1
 ...
 ElseIf(Left(objExcel.Cells(rows,1).Value, 9) =  "PPM [Add]") Then
 ...
 ElseIf(Left(objExcel.Cells(rows,1).Value, 27) =  "SPOTFIRE APPLICATIONS [Add]") Then
